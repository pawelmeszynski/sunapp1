...
class ConsoleIO extends BaseConsoleIO
{
    ...
    public function writeInfo($messages, $newline = true, $verbosity = self::NORMAL)
    {
        ...
        if (!$this->info_section) {
            $this->info_section = $this->console->section();
            $this->info_section->setFormatter($this->output->getFormatter());
        }
        $this->info_section->clear();
        $this->info_section->write($messages);
    }
    ...
    public function createProgressBar($max = 0)
    {
        ...
        $progress = new ProgressBar($section, $max, 0.01);
