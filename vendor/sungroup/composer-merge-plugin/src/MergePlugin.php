...
use SunGroup\Composer\Merge\ExtraPackage;
use SunGroup\Composer\Merge\MissingFileException;
use SunGroup\Composer\Merge\PluginState;
...
class MergePlugin implements PluginInterface, EventSubscriberInterface
{
     ...
     */
    const PACKAGE_NAME = 'sungroup/composer-merge-plugin';
    ...
    public function onInit(BaseEvent $event)
    {
        ...
        $this->merge_package = $localRepository->findPackage('sungroup/composer-merge-plugin', '*');
        if ($this->merge_package && !method_exists($this->merge_package, 'getAliasOf')) {
            $extra = $this->merge_package->getExtra();
            unset($extra['laravel']);
            $this->merge_package->setExtra($extra);
        }

        $packages = $localRepository->getPackages();
        foreach ($packages as $package) {
            $extras = $package->getExtra();
            if (isset($extras['merge-plugin']) && isset($extras['merge-plugin']['include'])) {
                $includes = $extras['merge-plugin']['include'];
                $this->mergeFiles($includes, false);
            }
        }
        ...
        if ($this->state->isComposer1()) {
            ...
            $localRepository->write($this->state->isDevMode(), $this->composer->getInstallationManager());
        }
    }
    ...
    protected function mergeFile(RootPackageInterface $root, $path)
    {
        ...
        $laravel = $package->getLaravel();
        if($laravel && is_array($laravel)) {
            $this->merge_package->setExtra(array_merge_recursive($this->merge_package->getExtra(), ['laravel'=>$laravel]));
        }
