                            ...
                            <div class="sidebar-menu-list">
                                <div class="list-group list-group-messages font-medium-1">
                                    <div @click="getItems('all', searchPhrase, 'click')"
                                        ...
                                        @lang('core::groups.all') <span class="badge badge-primary badge-pill float-right">@{{ itemsTotal }}</span>
                                    </div>
                                    ...
                                    <div class="sg-user-list list-group">
                                        ...
                                        <div :class="{'dd dd-main' : itemsFilter === 'all' && searchPhrase === ''}" data-token="{{ csrf_token() }}">
                                            <ol class="users-list-wrapper media-list p-0 dd-list" v-show="items.length">
                                                <li v-for="(item, index) in items" :key="item.id" class="list-item media dd-item" :data-id="item.id" :data-parent="item.attributes.parent_id" ref="nested" @dblclick.stop="itemsFilter == 'trashed' ? showElement(item.links.show) : editElement(item.links.edit)">
                                                    <div class="media-left pr-50">
                                                        ...
                                                        <template v-if="!item.attributes.core">
                                                            <div class="dd-handle has-tooltip has-tooltip--top has-tooltip--left" v-if="itemsFilter === 'all' && !searchedWithPhrase">
                                                                <i class="feather icon-move"></i>
                                                                <span class="tooltip-text">@lang('core::actions.move')</span>
                                                            </div>
                                                        </template>
                                                        <div class="dd-subtree has-tooltip has-tooltip--top has-tooltip--left"
                                                                v-if="itemsFilter === 'all' && item.attributes.children_count"
                                                             @click.stop="getSubItems(item.id)" @dblclick.stop.prevent="">
                                                            <i class="feather icon-corner-right-down"></i>
                                                            <span class="tooltip-text">@lang('core::actions.show_children')</span>
                                                        </div>
                                                    </div>
                                                            ...
                                                            <div class="mail-meta-item">
                                                                <div class="float-right">
                                                                    <div class="float-right todo-item-action d-flex">
                                                                        ...
                                                                        <a v-if="item.links.edit && !item.attributes.deleted_at && !item.attributes.core" :href="item.links.edit" class="edit-element has-tooltip has-tooltip--down" @click.prevent="editElement(item.links.edit)">
                                                                            <span class="tooltip-text">@lang('core::actions.edit')</span>
                                                                            <i class="m-25 feather icon-edit-2"></i>
                                                                        </a>
                                                                        <template v-if="!item.attributes.default && item.links.destroy && !item.attributes.core">
                                                        ...
                                                        <div class="mail-message">
                                                            <p class="list-group-item-text truncate mb-0">
                                                                <template v-if="!item.attributes.deleted_at">
                                                                    <div class="chip-wrapper d-inline-block" v-if="item.attributes.core">
                                                                        <div class="chip mb-0">
                                                                            <div class="chip-body">
                                                                                <span class="chip-text"><span class="bullet bullet-success bullet-xs"></span> @lang('core::groups.core')</span>
                                                                            </div>
                                                                        </div>
                                                                    </div>
                                                                </template>
...
@theme_asset('groups-vue', '../modules/core/assets/js/groups.js', ['base-vue'])
