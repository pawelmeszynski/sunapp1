        ...
        <div class="sg-detail-header">
            <div class="sg-header-left d-flex align-items-center mb-1">
                ...
                <h3 v-if="elementToShow">
                    ...
                    <span>@{{ elementToShow.attributes.title }}</span>
                </h3>
                ...
                <div class="col-12">
                    ...
                    <div class="tab-content">
                        <div class="tab-pane active" id="info" aria-labelledby="info-tab" role="tabpanel">
                            ...
                            <div>
                                @php
                                    $ability_names = ['show', 'create', 'edit', 'destroy', '*'];
                                @endphp
                                @foreach($abilities as $key => $module_ability)
                                    <h4>{{trans($module_ability['name']??$key)}}</h4>
                                    @if(isset($module_ability['items']))
                                    <div>
                                        @foreach($module_ability['items'] as $model => $model_translation)
                                            @if(is_string($model_translation))
                                                <h6>{{trans($model_translation)}}</h6>
                                                        ...
                                                        @foreach($ability_names as $ability_name)
                                                            <td>
                                                                <div class="form-control-plaintext custom-control custom-checkbox">
                                                                    <input id="abilities[{{$model}}][{{$ability_name}}]" type="checkbox" value="1"
                                                                           name="abilities[{{$model}}][{{$ability_name}}]" class="custom-control-input"
                                                                            data-ability="{{$ability_name}}" @change="checkAbilities($event);requiredAbilities($event)">
                                                                    <label for="abilities[{{$model}}][{{$ability_name}}]" class="custom-control-label">
                                                                        @lang('core::roles.'.$ability_name)
                                                                    </label>
                                                                </div>
                                                            </td>
                                                        @endforeach
                                            ...
                                            @else
                                                <h6>{{trans($model)}}</h6>
                                                <table class="table table-striped table-borderless table--abilities">
                                                @foreach($model_translation as $model => $model_translation)
                                                    @if(is_string($model_translation))
                                                        <tr>
                                                            <td colspan="5"><h6 style="margin: 0;">{{trans($model_translation)}}</h6></td>
                                                        </tr>
                                                        <tr data-model="{{$model}}">
                                                            @foreach($ability_names as $ability_name)
                                                                <td>
                                                    ...
                                                    @else
                                                        <tr>
                                                            <td colspan="5"><h6 style="margin: 0;">{{trans($model)}}</h6></td>
                                                        </tr>
                                                        @foreach($model_translation as $model => $model_translation)
                                                                @if(is_string($model_translation))
                                                                    <tr data-model="{{$model}}">
